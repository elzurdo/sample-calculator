...
stage_planning = "💰 plan a sample budget"
...
if stage_landing != audit_stage:
    ...
    st.image(image_differences, caption="Visualising the three types of calculators.  The bell shapes represent the model probability given sample data (known as the posterior distribution).")
    ...
    * **Model Comparison** - each model has its own sample, i.e: 2 sample sizes and 2 success rates.
    ...
    text_learn_more = """
    There are two good rules of thumb when relating sample results to the model:
    * The sample result is a **proxy** for that of the model. Not equal.
...
if option_accuracy == calculator_type:
    if stage_planning == audit_stage:
        ...
        if planning_all == explore_mode:
            ...
            f"""    
            ### Instructions  
    ⬅️ On the left hand panel provide the **Baseline {metric_name_title} Rate** expected and **Goal Accuracy** to find out the minimum **Sample Size**.
    """
            ...
            f"""### Result   
The minimum **Sample Size** required to measure a **{metric_name_title} Rate** of {benchmark_percent}% within {result['accuracy'] * 100.:0.1f}% **Accuracy** (95% credible interval) is:  
...
"""
        ...
        f"""### Result   
An sample size of {sample_size:,} with a **{metric_name_title} Rate** of {observed_success_rate * 100.:0.1f}% was most likely to be generated by a model with a {metric_name} rate:
# between {ci_min * 100:0.1f}% and {ci_max * 100:0.1f}%
({CI_FRACTION * 100.:0.1f}% credible interval of full width **{(ci_max - ci_min) * 100:0.1f}%**)
"""
    ...
    * **{metric_name_title} Rate** - This assumes metrics that are binary in nature (only has value "success", "failure") and follow a Bernoulli distribution.
    ...
    observed_success_percent = st.sidebar.number_input(f'Sample {metric_name.title()} Rate (%)',
    ...
    if audit_stage in [stage_interpreting]:
        sample_size = st.sidebar.number_input('Sample Size', value=100, min_value=MIN_AUDIT_SIZE_CLEARANCE,
                                              max_value=10000)
...
the observable **Sample {metric_name_title} Rate** is at least {observed_success_percent}% requires"""
...
Please adjust either the **Risk Factor** or the **Sample {metric_name_title} Rate.**"""
    ...
    {mfpr_percent}%, results with with a higher **Sample FPR** will not be considered >{success_rate_boundary*100.:0.1f}% {metric_name} and hence fail. 
    ...
    E.g, this commitment guarantees that for every 1,000 samples, we should expect and average of 
    ...
    input_str = "# Input"
    input_str += f"\n**Model A** has a sample size of {sample_size_a:,} with a sample {metric_name_title} rate of {observed_success_percent_a:0.1f}%"
    input_str += f"\n\n**Model B** has a sample size of {sample_size_b:,} with a sample {metric_name_title} rate of {observed_success_percent_b:0.1f}%"
    st.write(input_str)

    if ci_max_b < ci_min_a:
        result_str = f"**Model A**'s actual {metric_name_title} rate is likely to be better than **B**'s with over {CI_FRACTION*100:0.1f}% credibility."
        reason_str = f"Because CI Max of **Model B** of {ci_max_b * 100:0.1f}% is smaller than CI Min of **A** {ci_min_a * 100:0.1f}%."
    elif ci_max_a < ci_min_b:
        result_str = f"**Model B**'s actual {metric_name_title} rate is likely to be better than **A**'s with over {CI_FRACTION * 100:0.1f}% credibility."
        reason_str = f"Because CI Max of **Model A** of {ci_max_a * 100:0.1f}% is smaller than CI Min of **B** {ci_min_b * 100:0.1f}%."
    else:
        result_str = f"**Models A**'s and **B**'s actual {metric_name_title} rate cannot be distinguished with {CI_FRACTION * 100:0.1f}% credibility."
        reason_str = None

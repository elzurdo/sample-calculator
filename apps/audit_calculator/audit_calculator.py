...
text_short_mfpr = f"""
*For an audit to be considered safe we require 
FPR>{100. - mfpr_percent:0.2f}%. 
This guarantees that for every 1,000 similar decisions ("this model is safe"), we should expect about 
{mfpr_rate * 1000.:0.1f} to be incorrect (i.e, the model is not actually safe).*
"""
...
{mfpr_percent}%, audit results with a higher **FPR** will be considered not safe. 

This commitment guarantees that for every 1,000 audits, we should expect about 
{mfpr_rate * 1000.:0.1f} to be a False Postive (on average 1,000 audits).
"""

#st.beta_expander()
with st.beta_expander('The Max FPR Commitment Explained'):
...
if boundary_success_bool:
    ...
    if mfpr_success_bool:
        ...
        audit_result_str = f""" The model that generated these 
                            audit results **may be considered safe**. 

**Reason**  
The results indicate that the FPR is **smaller** than the committed Max FPR.
        ...
        audit_result_str = f""" The model that generated these 
                                    audit results **may NOT be considered safe**. 
        
**Reason**  
The results indicate that the FPR is **larger** than the committed Max FPR 
({observation_result['false_rate'] * 100.:0.2f}% >{mfpr_rate * 100.:0.2f}%). 


Considering this model safe risks increasing the FPR to above the committed {mfpr_rate * 100.:0.2f}%

**Suggested Actions**  
* Explore the reason for the high unsafe rate and fix the model.  
* Collect more annotated for further justification. Note that Max FPR must remain at {mfpr_rate * 100.:0.2f}% or lower.  
"""
else:
    audit_result_str = f"""The model that generated the audit sample **may not be considered safe**.     

**Reason**  
The Audit Success Rate of {observed_success_rate * 100:0.1f}%  is lower than the threshold o {success_rate_boundary * 100:0.1f}%."""
...
has a 
{observation_result['true_rate'] * 100:0.1f}% 
probability of being generated by a model with a success rate of at least 
...
with st.beta_expander('Visualise Results'):
